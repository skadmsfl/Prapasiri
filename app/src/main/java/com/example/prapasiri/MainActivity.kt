...
class MainActivity : AppCompatActivity() {
    ...
    fun showAllUser(v: View){
        var user = usersDBHelper.readAllUsers()
        this.ll_entries.removeAllViews()
        users.forEach {
            var tv_user = TextView(this)
            tv_user.textSize = 30F
            tv_user.text =
                "Id: " + it.userid.toString()+"\n" + "Name: " + it.name.toString() + "  Age: " + it.age.toString()+"\n"
            this.ll_entries.addView(tv_user)
        }
        this.textview_reslt.text = "Fetched " + users.size + " users"
